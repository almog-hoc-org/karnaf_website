...
const concernInsight: Record<QuizConcern, string> = {
  overpay:
    "הפחד לשלם יותר מדי נפתר בשיטה, לא בתחושת בטן: בפרק הרלוונטי לומדים להשוות את הנכס לעסקאות שנסגרו באותו רחוב — ולדעת אם המחיר מוצדק לפני שמציעים.",
  mortgage:
    "משכנתא היא לא כישרון מולד. בפרק המימון תבינו מסלולים, תמהיל וריביות — ותגיעו לבנק עם השאלות שהופכות אתכם ללקוח שקשה למכור לו.",
  "where-to-start":
    "התוכנית בנויה בדיוק בשביל זה: 15 פרקים בסדר שבו תפגשו אותם במציאות — מהתקציב, דרך החיפוש והבדיקות, ועד החתימה.",
  negotiation:
    "מו״מ טוב הוא לא כריזמה — הוא נתונים. בפרק המשא ומתן תלמדו מתי ללחוץ, מתי לשתוק, ואיך להיכנס לשיחה עם המספרים שמחזיקים את העמדה שלכם.",
};
