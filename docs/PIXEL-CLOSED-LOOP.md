...
## סטטוס נכון ל-2026-09-05 (נבדק מול Events Manager דרך ה-API)

- **Purchase כבר נסגר.** הפיקסל מותקן ב-`my.schooler.biz` ואירועי `Purchase` מתקבלים
  משם (למשל 2 רכישות ב-26.8.2026). אין צורך להדביק שוב את הסניפט — רק לוודא
  שהוא נשאר במקום אם דף התודה ב-Schooler משתנה.
- **Lead** נורה מהקוד בלבד (`trackLead` ב-`src/lib/pixel.ts`) עם `value` יחסי לפי
  מקור. חוק ה-Event Setup "Lead על לחיצת כפתור 'בואו נדבר'" (rule `1047336058238510`)
  הושבת — הוא ירה גם על כפתור הוואטסאפ בניווט ועל שליחה שנכשלה.
- **InitiateCheckout** נורה מהקוד עם `value`/`content_ids`. שבעה חוקי Event Setup
  מבוססי-טקסט-כפתור שכפלו אותו והושבתו (rules `2515485282209027`, `3598883460269668`,
  `1772905730504857`, `1760771265344462`, `1759915111686101`, `1339926501632556`,
  `1026393763523212`). כולם הפיכים: Events Manager → Event Setup → Activate.
- **וובינר**: הפיקסל פעיל גם ב-`webinar.karnafnadlan.com` / `webinarkarnaf.ravpage.co.il`,
  ו-`CompleteRegistration` נורה על דף `/registration` (חוק `1495026288616269`).
- **עדיין פתוח**: `VITE_GA4_ID` ב-Vercel (אין גישה מכאן). Custom Conversions לא
  הוגדרו — לא חובה: `Purchase` ו-`Lead` הם אירועים סטנדרטיים וניתן לבחור בהם
  ישירות כיעד אופטימיזציה בקמפיין.
- חוק custom "התעניין בתוכנית" (rule `1987209401777842`) נורה על **כל** URL באתר
  (`contains https://www.karnafnadlan.com/`) — כ-370 אירועים ב-28 יום, כפילות של
  PageView. לא הושבת (נוצר ידנית); אם לא משמש לקהל — כדאי להשבית.
